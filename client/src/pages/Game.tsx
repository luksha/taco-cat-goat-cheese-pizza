    ...
    <div className="min-h-screen w-full bg-background flex flex-col overflow-hidden relative">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 gap-8">
        ...
        <div className="text-center space-y-2">
          <div className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Current Word</div>
          <motion.div 
            key={chantIndex}
        ...
        <div className="relative flex justify-center items-center py-8">
           ...
           <div className="z-10 relative">
             <GameCard 
               ...
               isFlipped={isFlipped} 
               className={feedback === "match" ? "ring-8 ring-green-400 rounded-3xl" : feedback === "wrong" ? "ring-8 ring-red-400 rounded-3xl" : ""}
             />
           </div>
        </div>
      ...
      <Dialog open={showNameDialog} onOpenChange={setShowNameDialog}>
        <DialogContent className="sm:max-w-md border-4 border-primary/20 rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-center text-3xl font-display font-black text-primary">GAME OVER</DialogTitle>
          </DialogHeader>
          <div className="text-center py-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-muted p-4 rounded-2xl">
                <div className="text-sm text-muted-foreground font-bold">FINAL SCORE</div>
                <div className="text-3xl font-black">{score}</div>
              </div>
              <div className="bg-muted p-4 rounded-2xl">
                <div className="text-sm text-muted-foreground font-bold">MAX STREAK</div>
          ...
          <DialogFooter className="sm:justify-center gap-2">
            <Button variant="secondary" onClick={() => setLocation("/")}>Skip</Button>
            <Button onClick={handleScoreSubmit} disabled={!username || submitScore.isPending}>
